...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($email) || !empty($phone)) {
        ...
        if (!empty($cDataForm->name_model)) {
            $cDataForm->name_model=get_the_title($cDataForm->name_model) . " (" .
            get_permalink($cDataForm->name_model) . ")";
        }
...
?>
                                            ...
                                            <div class="left-div">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control w150 madel-dropdown"
                                                        ...
                                                        foreach ($directory as $key => $value) {
                                                            echo '<option img="' . $value['url'] .
                                                                '" value="' . $key . '" ' .
                                                                isSelected($key, $directory_id) . '>' .
                                                                $value['title'] . '</option>';
                                                        }
                                                        ?>
                                                    </select>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control w100 duration-dropdown" name="duration"
                                                        ...
                                                        $id_term = ConfigurationParameters::
                                                        getTermIdByName('Duration', 'category');
                                                        $arr_terms_duration = ConfigurationParameters::
                                                        getTermsList('category', $id_term);
    ...
    <script>
        $(function() {
            function setModelSelected($id_model) {
    
                const elementSelected = document.getElementById('model_name');
                elementSelected.value = $id_model;
                booking.modelChange(elementSelected);
        
            }
    
            // get model's id from URL
            let searchParams = new URLSearchParams(window.location.search);
            let id_model = searchParams.get('model_id');
    
            // if id was found, select model
            if (/^\d+$/.test(id_model)) {
                setModelSelected(id_model);
            }
        });
