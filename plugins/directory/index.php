...
register_activation_hook( __FILE__, 'dr_activate_pluign');
add_action( 'init', 'directory_init' );
add_action( 'save_post_directory', 'dr_save_post_admin', 10, 3);
add_filter( 'the_content', 'dr_filter_directory_content' );
 ...
 * Register Meta Boxes
 */
function rates_register_meta_boxes() {
    add_meta_box( 'rates-1', __('Rates', 'rates'), 'rates_display_callback', 'directory' );
}
add_action( 'add_meta_boxes', 'rates_register_meta_boxes' );

/*
 * Meta Boxes display callback
 * @param WP_Post $post Current post object
 */
function rates_display_callback( $post ){
 ...
 */
function rates_save_meta_box( $post_id ) {
    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
    if ( $parent_id = wp_is_post_revision( $post_id ) ) {
        $post_id = $parent_id;
    }
    $fields = [
        'add_hour_in', 'add_hour_out',
        'one_hour_in', 'one_hour_out', 
        'dinner_date_in', 'dinner_date_out', 
        'overnight_in', 'overnight_out', 
    ];
...
add_action( 'save_post', 'rates_save_meta_box' );




/*
 * Photo Gallery Metabox + Image Cropper + Image Swap
 */
/**
...
add_filter( 'intermediate_image_sizes', function( $sizes ) {
    return array_filter( $sizes, function( $val ){
        return 'medium' !== $val; // Filter out 'medium_large'
    } );
} );

add_filter( 'intermediate_image_sizes', function( $sizes ) {
    return array_filter( $sizes, function( $val ){
        return 'post-thumbnail' !== $val; // Filter out 'medium_large'
    } );
} );

add_filter( 'intermediate_image_sizes', function( $sizes ) {
    return array_filter( $sizes, function( $val ){
        return 'large' !== $val; // Filter out 'medium_large'
    } );
} );
...
final class WP_Term_Order {
    ...
    public function enqueue_scripts() {
        wp_enqueue_script( 'term-order-quick-edit', $this->url . 'assets/quick-edit.js', array( 'jquery' ), $this->db_version, true );

        // Enqueue fancy ordering
        if ( true === $this->fancy ) {
            wp_enqueue_script( 'term-order-reorder', $this->url . 'assets/reorder.js', array( 'jquery-ui-sortable' ), $this->db_version, true );
        }
    }
...
*/
    ...
    include_once(CPTPATH . 'includes/class.cpto.php');
    include_once(CPTPATH . 'includes/class.functions.php'); 
